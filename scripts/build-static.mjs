...
// from deployed HTML unless a page explicitly declares public market prices.
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) await copyDirectory(sourcePath, outputPath);
      ...
      if (entry.name.toLowerCase().endsWith(".html")) {
        ...
        const exposesPublicMarketPrices = /<html\b[^>]*\bdata-public-market-prices\b/i.test(html);
        const deployed = exposesPublicMarketPrices
          ? html
          : html.includes("</head>")
